...
NPP_regions_df <- as.data.frame(NPP_regions) %>% #make into dataframe
  ...
  summarise(global_NPP_2024 = mean(global_NPP_2024, na.rm = TRUE))
...
ABG_regions_df <- as.data.frame(ABG_regions) %>% #make into dataframe
  ...
  group_by(ECO_NAME, BIOME_NAME) %>% #group by ecoregion and biome
  summarise(aboveground_biomass_carbon_2010 = mean(aboveground_biomass_carbon_2010, na.rm = TRUE)) %>% #mean for ecoregion
  mutate(ABG_Pg = aboveground_biomass_carbon_2010 / 10000000) #convert to Pg C per km^2
...
AM_roots_ecoregions_df <- as.data.frame(AM_roots_ecoregions) %>% #make into dataframe
  ...
  group_by(ECO_NAME, BIOME_NAME) %>% #group by ecoregion and biome
  summarise(AM_roots_colonized = mean(AM_roots_colonized, na.rm = TRUE)) %>% #mean for ecoregion
  mutate(AM_roots_Pg = AM_roots_colonized / 10000000) #convert to Pg C per km^2
...
#Overlay ecoregions onto raster
AM_int_sf <- st_as_sf(AM_df, coords = c("x", "y"), crs = st_crs(AM_roots)) #convert to sf object
AM_int_sf <- st_transform(AM_sf, crs = 3395) #convert coord system to EPSG:3395 for World Mercator
AM_int_sf <- st_make_valid(AM_sf) #fix invalid geometries
...
AM_int_ecoregions_df <- as.data.frame(AM_int_ecoregions) %>% #make into dataframe
  dplyr::select(ECO_NAME, BIOME_NAME, geometry, AM_roots_colonized) %>% #select relevant columns
  group_by(ECO_NAME, BIOME_NAME) %>% #group by ecoregion and biome
  summarise(AM_intensity = mean(AM_roots_colonized, na.rm = TRUE)) #mean for ecoregion
...
bio_regions <- merge(ecoregions_df, ABG_regions_df, by = c("ECO_NAME", "BIOME_NAME")) %>%
  ...
  mutate(ABG_Pg = ABG_Pg * area_sq_km, #calculate ABG C in Pg per sq km
         AM_roots_Pg = AM_roots_Pg * area_sq_km) %>% #calculate AM C in Pg per sq km
  replace(is.na(.), 0) #replace NA values with 0

#Create new column with more specific ecoregion names
bio_regions <- bio_regions %>%
  filter(ABG_Pg > 0) %>% #remove 0 values
...
#Summarise by biome
bio_regions_sum <- bio_regions %>%
  group_by(biome) %>% #group by biome
  dplyr::select(biome, area_sq_km, ABG_Pg, AM_roots_Pg) %>% #select only relevant columns
  summarise_at(c("area_sq_km", "ABG_Pg", "AM_roots_Pg"), sum, na.rm = TRUE) #%>%
