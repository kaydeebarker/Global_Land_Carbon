...
#View map
#ggplot(data = ecoregions) + geom_sf()
...
ecoregions_area_sf <- st_as_sf(ecoregions_area, coords = c("geometry"), crs = 3395)
...
ABG_regions <- st_join(ABG_sf, ecoregions_area_sf, join = st_intersects) #join ecoregions to ABG data
...
biomass_regions <- st_join(BG_sf, ABG_regions, join = st_intersects) #join ecoregions, ABG, and BG data

biomass_regions_df <- as.data.frame(biomass_regions) #make into dataframe
...
biomass_regions_df <- biomass_regions_df %>%
  dplyr::select(ECO_NAME, BIOME_NAME, area_sq_km, aboveground_biomass_carbon_2010, belowground_biomass_carbon_2010) #%>% #select only relevant columns
  #na.omit() %>%
