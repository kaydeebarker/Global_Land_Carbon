...
I used a mycorrhizal fine root stocks spatial dataset by [Milagros et al. 2023](https://www.nature.com/articles/s41597-022-01913-2) hosted on [DRYAD](https://doi.org/10.5061/dryad.866t1g1tt).

Aboveground biomass C was taken from a NASA EarthData-hosted dataset, Global Aboveground and Belowground Biomass Carbon Density Maps for the Year 2010 ([Spawn and Gibbs, 2020](https://doi.org/10.3334/ORNLDAAC/1763)). 


All C data is in petagrams (Pg) of C.
...
#ecoregions_area_df <- as.data.frame(ecoregions_area, xy = TRUE)
...
#Make df and remove 0 values
AM_df <- as.data.frame(AM_roots, xy=TRUE) %>%
  filter(AM_roots_colonized > 0) #remove 0 values


#Overlay ecoregions onto raster
AM_sf <- st_as_sf(AM_df, coords = c("x", "y"), crs = st_crs(AM_roots)) #convert to sf object
AM_sf <- st_transform(AM_sf, crs = 3395) #convert coord system to EPSG:3395 for World Mercator
AM_sf <- st_make_valid(AM_sf) #fix invalid geometries
AM_ecoregions <- st_join(ecoregions_sf, AM_sf, join = st_intersects) #join ecoregions to AM data
...
#Make into dataframe and select columns
AM_ecoregions_df <- as.data.frame(AM_ecoregions) %>% #make into dataframe
...
crop_sqkm #update use calculated irrigated cropland from above
