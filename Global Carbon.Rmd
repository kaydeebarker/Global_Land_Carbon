...
OneEarth: https://www.oneearth.org/

I used data from [Steidinger et al. 2019](https://doi.org/10.1038/s41586-019-1411-2) to determine the proportion of AM vs. ectomycorrhizal (EcM) fungal trees. 

I used a grass:tree ratio of 3.56 for savannas, converted to area of 78% grass, 22% tree, before applying the AM to EcM rate for trees ([Belovitch et al. 2023](https://doi.org/10.1111/nph.18909))
...
ecoregions_area <- ecoregions_area %>%
  ...
  mutate(AM_area = #multiply by proportion of AM
           case_when(biome == "Tundra"
                     ...
                     ~ 0,
            .default = area_sq_km)) %>%
  mutate(EM_area = #multiply by proportion of AM
           case_when(biome == "Tundra"
                     ~ (area_sq_km*((Steidinger$median[
...
ecoregions_sum <- ecoregions_area %>%
  ...
  select(biome, area_sq_km, AM_area, EM_area) %>% #select only relevant columns
  summarise_at(c("area_sq_km", "AM_area", "EM_area"), sum, na.rm = TRUE) #%>%
...
ecoregions_area %>%
  na.omit() %>%
ggplot(, aes(x=biome, y=AMbio_C, fill=biome)) + 
