...
Biomass C from a NASA EarthData-hosted dataset, Global Aboveground and Belowground Biomass Carbon Density Maps for the Year 2010 ([Spawn and Gibbs, 2020](https://doi.org/10.3334/ORNLDAAC/1763)).
...
#Visualize
ggplot() +
  geom_sf(data = ecoregions_sf, aes(fill = BIOME_NAME)) +
  theme_minimal() +
...
```{r load cropland data}

#Download cropland cover map from Global Land Analysis & Discovery if not downloaded already
...
#Read in and lower resolution for processing
cropland <- rast("Spatial_data/cropland_2019.tif") %>%
  aggregate(, fact = 3.33) #aggregate to 10km resolution

#Make df and remove 0 values
cropland_df <- as.data.frame(cropland, xy=TRUE) %>%
  filter(cropland_2019 > 0) #remove 0 values
...
#Visualize
ggplot() +
  geom_sf(data = cropland_sf, aes(fill = cropland_2019)) +
  scale_fill_viridis_c() +
  theme_minimal() +
  theme(legend.position = "bottom")

#Make into dataframe and select columns
#crop_ecoregions_df <- as.data.frame(crop_ecoregions) %>% #make into dataframe
#  dplyr::select(ECO_NAME, BIOME_NAME, area_sq_km, geometry, cropland_2019) #select relevant columns
...
cropland_sf <- st_make_valid(ABG_sf) #fix invalid geometries
ABG_regions <- st_join(ABG_sf, ecoregions_sf, join = st_intersects) #join ecoregions to ABG data
...
cropland_sf <- st_make_valid(BG_sf) #fix invalid geometries
...
# Check CRS of all spatial objects
st_crs(ecoregions_area_sf)
st_crs(cropland_sf)
st_crs(ABG_sf)
st_crs(BG_sf)
...
# Check extents
library(raster)
extent(ecoregions_area_sf)
extent(cropland_sf)
extent(ABG_sf)
extent(BG_sf)

#Combine data from ABG and BG
biomass_regions_sf <- st_union(ABG_sf, BG_sf) #%>% #join ecoregions to ABG and BG data
  st_union(cropland_sf) #join ecoregions to cropland data
