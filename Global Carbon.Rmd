...
- I first used the United Nations Food and Agriculture Organization (FAO)'s Global Land Cover - SHARE database ([Latham et al. 2014](https://www.fao.org/uploads/media/glc-share-doc.pdf)). Aboveground carbon (C) and root C were taken from previous C estimates and root:shoot ratios ([Robinson 2007](https://royalsocietypublishing.org/doi/10.1098/rspb.2007.1012), [Liu et al. 2015](https://www.nature.com/articles/nclimate2581)). For cropland cover, I used data from [Global Land Analysis & Discovery](https://glad.umd.edu/dataset/croplands) ([Potapov et al. 2022](https://doi.org/10.1038/s43016-021-00429-z)).
...
Update: I used data from [Ecoregions2017](https://developers.google.com/earth-engine/datasets/catalog/RESOLVE_ECOREGIONS_2017) to determine total area in square kilometers for each biome. I used the [Global Food Security Support Analysis Data (GFSAD)](https://lpdaac.usgs.gov/products/gfsad1kcdv001/#tools) ([Huang et al. 2016](https://doi.org/10.1016/j.rse.2015.12.036)) to determine the area of cropland cover.

I used a mycorrhizal fine root stocks spatial dataset by [Milagros et al. 2023](https://www.nature.com/articles/s41597-022-01913-2) hosted on [DRYAD](https://doi.org/10.5061/dryad.866t1g1tt). Aboveground biomass C was taken from a NASA EarthData-hosted dataset, Global Aboveground and Belowground Biomass Carbon Density Maps for the Year 2010 ([Spawn and Gibbs, 2020](https://doi.org/10.3334/ORNLDAAC/1763)). I added net primary productivity (NPP) data for one year from NASA Earth Observations ([NEO](https://neo.gsfc.nasa.gov/view.php?datasetId=MOD17A3H_Y_NPP)) Annual Terra Moderate Resolution Imaging Spectroradiometer (MODIS) dataset ([Running and Zhao, 2021](https://doi.org/10.5067/MODIS/MOD17A3HGF.061)). The original unit of these datasets were megagrams (Mg) of C per hectare (ha) or grams (g) per square meter; I converted all units to petagrams (Pg) of C per square kilometer (km<sup>2</sup>).


Final C data is in petagrams (Pg) of C.

### Ecosystem Cover Data

```{r load FAO data, eval=FALSE}
...
landcover <- rast("glc_shv10_DOM.Tif")

#Make df of frequency of each value in raster
landcover <- as.data.frame(freq(landcover)) %>%
...
ecoregions <- st_read("Spatial_data/Ecoregions2017.shp")
...
ecoregions_sf <- st_transform(ecoregions, crs = 3395) # EPSG:3395 for World Mercator

#Convert to sf object for spacial (need for overlay)
ecoregions_sf <- st_as_sf(ecoregions_sf, coords = c("geometry"), crs = 3395)

```

### Cropland Cover

```{r load cropland data, eval=FALSE}

#Download cropland cover map from Global Land Analysis & Discovery if not downloaded already - update don't use
if(file.exists('Spatial_data/cropland_2019.tif')){
  print('downloaded')
  }else{
  download.file("https://glad.geog.umd.edu/Potapov/Global_Crop/Data/Global_cropland_3km_2019.tif", 
                destfile= "Spatial_data/cropland_2019.tif", method= "libcurl", 
                quiet = FALSE, mode = "w", cacheOK = TRUE,
                options(timeout = max(300, getOption("timeout"))))
  }

#Read in cropland cover map from Global Land Analysis & Discovery
cropland2019 <- rast("Spatial_data/cropland_2019.tif") %>%
  aggregate(, fact = 3.33) #aggregate to ~10km resolution

#Read in raster from Global Food Security Support Analysis Dataset and lower resolution for processing
cropland <- rast("Spatial_data/globalcropland2010.tif") 
  
#Filter values to keep, set all other cells to NA
crops_irr <- c(1,2,3) #set values to keep in raster - irrigated cropland only
cropland_irr <- mask(cropland, cropland %in% crops_irr, maskvalue=FALSE) #mask raster to keep only values in crops_keep
crop_sqkm <- global(!is.na(cropland_irr), fun = "sum", na.rm=TRUE)[1,1] #get the number of non-NA cells (which are 1 sq. km)
crop_sqkm <- as.data.frame(crop_sqkm) #make into df
write_csv(crop_sqkm, "crop_sqkm.csv")

cropland_irr <- cropland_irr %>%
  aggregate(, fact = 10) #aggregate to 10km resolution

#Visualize raster
#plot(cropland_irr, main = "Global Cropland Cover 2010") #plot raster
  

```

```{r cropland to sf, eval=FALSE}

#Make df and remove 0 values
cropland_df <- as.data.frame(cropland_irr, xy=TRUE) 

#Overlay ecoregions onto raster
cropland_sf <- st_as_sf(cropland_df, coords = c("x", "y"), crs = st_crs(cropland)) #convert to sf object
cropland_sf <- st_transform(cropland_sf, crs = 3395) #convert coord system to EPSG:3395 for World Mercator
cropland_sf <- st_make_valid(cropland_sf) #fix invalid geometries
crop_ecoregions <- st_join(ecoregions_sf, cropland_sf, join = st_intersects) #join ecoregions to cropland data

#Make into dataframe and select columns
#cropland_df <- as.data.frame(crop_ecoregions) %>% #make into dataframe
 # dplyr::select(ECO_NAME, BIOME_NAME, geometry, globalcropland2010) #%>% #select relevant columns
  #mutate(BIOME_NAME =
   # case_when(globalcropland >0 & BIOME_NAME = NA, ~ "Cropland"))

#Visualize
#ggplot() +
 # geom_sf(data = ecoregions_sf, aes(fill = BIOME_NAME), color = NA) +
  #geom_sf(data = cropland_sf, aes(fill = globalcropland2010), color = NA) +
  #theme_minimal() +
  #labs(title = "Cropland Cover in Ecoregions") +
...
#Download NPP data from NASA Earth Observations if not downloaded already
if(file.exists('Spatial_data/global_NPP_2024.tif')){
  print('downloaded')
  }else{
  download.file("https://neo.gsfc.nasa.gov/servlet/RenderData?si=2011916&cs=gs&format=TIFF&width=3600&height=1800", 
                destfile= "Spatial_data/global_NPP_2024.tif", method= "libcurl", 
                quiet = FALSE, mode = "w", cacheOK = TRUE,
                options(timeout = max(300, getOption("timeout"))))
  }

#Read in NPP data from NASA Earth Observations
NPP <- rast("Spatial_data/global_NPP_2024.tif") %>%
  aggregate(, fact = 3.33) #aggregate

#Make df and remove 0 values
NPP_df <- as.data.frame(NPP, xy=TRUE) %>%
  filter(global_NPP_2024 > 0) #remove 0 values

#Overlay ecoregions onto raster
NPP_sf <- st_as_sf(NPP_df, coords = c("x", "y"), crs = st_crs(NPP)) #convert to sf object
NPP_sf <- st_transform(NPP_sf, crs = 3395) #convert coord system to EPSG:3395 for World Mercator
NPP_sf <- st_make_valid(NPP_sf) #fix invalid geometries
NPP_regions <- st_join(ecoregions_sf, NPP_sf, join = st_intersects, left = FALSE) #join ecoregions to NPP data

#Make into dataframe and summarise, convert to Pg C per sq km (from g C per sq m)
NPP_regions_df <- as.data.frame(NPP_regions) %>% #make into dataframe
  dplyr::select(ECO_NAME, BIOME_NAME, geometry, global_NPP_2024) %>%
  group_by(ECO_NAME, BIOME_NAME) %>% #group by ecoregion and biome
  summarise(global_NPP_2024 = sum(global_NPP_2024, na.rm = TRUE)) %>% #sum for ecoregion
  mutate(NPP_Pg = global_NPP_2024 / 1000000000000) #convert to Pg C per km^2
...
#Read in aboveground biomass C data from NASA EarthData and lower resolution for processing
ABG_C <- rast("Spatial_data/aboveground_biomass_carbon_2010.tif") %>%
  #aggregate(, fact = 3.33) #%>% #aggregate 
  aggregate(, fact = 33.33) #aggregate

#Make df and remove 0s
ABG_C_df <- as.data.frame(ABG_C, xy=TRUE) %>%
  filter(aboveground_biomass_carbon_2010 > 0) #remove 0 values

#Overlay ecoregions onto raster
ABG_sf <- st_as_sf(ABG_C_df, coords = c("x", "y"), crs = st_crs(ABG_C)) #convert to sf object
ABG_sf <- st_transform(ABG_sf, crs = 3395) #convert coord system to EPSG:3395 for World Mercator
ABG_sf <- st_make_valid(ABG_sf) #fix invalid geometries
ABG_regions <- st_join(ecoregions_sf, ABG_sf, join = st_intersects, left = FALSE) #join ecoregions to ABG data

#Make into dataframe and summarise
ABG_regions_df <- as.data.frame(ABG_regions) %>% #make into dataframe
  dplyr::select(ECO_NAME, BIOME_NAME, geometry, aboveground_biomass_carbon_2010) %>%
  group_by(ECO_NAME, BIOME_NAME) %>% #group by ecoregion and biome
  summarise(ABG_mean = mean(aboveground_biomass_carbon_2010, na.rm = TRUE), 
            ABG_sum = sum(aboveground_biomass_carbon_2010,na.rm = TRUE)) %>% #mean for ecoregion
  mutate(ABG_Pg_mean = ABG_mean / 10000000) %>% #convert to Pg C per km^2
  mutate(ABG_Pg_sum = ABG_sum / 10000000) #convert to Pg C per km^2

```

#### Belowground Biomass Carbon

```{r load BG data, eval=FALSE}

#Read in belowground biomass C data from NASA EarthData and lower resolution for processing
BG_C <- rast("Spatial_data/belowground_biomass_carbon_2010.tif") %>%
  #aggregate(, fact = 3.33) #%>% #aggregate to 1km resolution
  aggregate(, fact = 33.33) #aggregate to 10km resolution

#Make df and remove 0s
BG_C_df <- as.data.frame(BG_C, xy=TRUE) %>%
  filter(belowground_biomass_carbon_2010 > 0) #remove 0 values

#Overlay ecoregions onto raster
BG_sf <- st_as_sf(BG_C_df, coords = c("x", "y"), crs = st_crs(BG_C)) #convert to sf object
BG_sf <- st_transform(BG_sf, crs = 3395) #convert coord system to EPSG:3395 for World Mercator
BG_sf <- st_make_valid(BG_sf) #fix invalid geometries
#BG_regions <- st_join(BG_sf, ecoregions_sf, join = st_intersects) #join ecoregions, ABG, and BG data

#Make into dataframe and select columns
#BG_regions_df <- as.data.frame(BG_regions) %>% #make into dataframe
#  dplyr::select(ECO_NAME, BIOME_NAME, area_sq_km, geometry, belowground_biomass_carbon_2010)
...
#Download cropland cover map from Global Land Analysis & Discovery if not downloaded already - update don't use
if(file.exists('Spatial_data/myc_map/AM_roots_colonized.tif')){
  print('downloaded')
  }else{
  download.file("https://datadryad.org/dataset/doi:10.5061/dryad.866t1g1tt#:~:text=Download,-full%20dataset", 
                destfile= "Spatial_data/myc_map.zip", method= "libcurl", 
                quiet = FALSE, mode = "w", cacheOK = TRUE,
                options(timeout = max(300, getOption("timeout"))))
unzip("Spatial_data/myc_map.zip", exdir = ".")
file.remove("Spatial_data/myc_map.zip")}


#Read in AM root colonization data - 10 arc-min resolution
AM_roots <- rast("Spatial_data/myc_map/AM_roots_colonized.tif") #%>%
  #aggregate(, fact = 3.33) #%>% #aggregate

#Make df and remove 0 values
AM_roots_df <- as.data.frame(AM_roots, xy=TRUE) %>%
  filter(AM_roots_colonized > 0) #remove 0 values


#Overlay ecoregions onto raster
AM_roots_sf <- st_as_sf(AM_roots_df, coords = c("x", "y"), crs = st_crs(AM_roots)) #convert to sf object
AM_roots_sf <- st_transform(AM_roots_sf, crs = 3395) #convert coord system to EPSG:3395 for World Mercator
AM_roots_sf <- st_make_valid(AM_roots_sf) #fix invalid geometries
AM_roots_ecoregions <- st_join(ecoregions_sf, AM_roots_sf, join = st_intersects) #join ecoregions to AM data

#Tried to put AM and crops together, but doesn't work because both have points instead of polygons
#AM_crops <- st_join(AM_sf, cropland_sf, join = st_intersects) #join cropland data to AM data

#Make into dataframe and summarise by ecoregion
AM_roots_ecoregions_df <- as.data.frame(AM_roots_ecoregions) %>% #make into dataframe
  dplyr::select(ECO_NAME, BIOME_NAME, geometry, AM_roots_colonized) %>% #select relevant columns
  group_by(ECO_NAME, BIOME_NAME) %>% #group by ecoregion and biome
  summarise(AM_roots_colonized_mean = mean(AM_roots_colonized, na.rm = TRUE), 
            AM_roots_colonized_sum = sum(AM_roots_colonized, na.rm = TRUE)) %>% #mean for ecoregion
  mutate(AM_roots_Pg_mean = AM_roots_colonized_mean / 10000000) %>% #convert to Pg C per km^2
  mutate(AM_roots_Pg_sum = AM_roots_colonized_sum / 10000000) #convert to Pg C per km^2
...
#Download cropland cover map from Global Land Analysis & Discovery if not downloaded already - update don't use
if(file.exists('Spatial_data/myc_map/AM_intensity_colonization.tif')){
  print('downloaded')
  }else{
  download.file("https://datadryad.org/dataset/doi:10.5061/dryad.866t1g1tt#:~:text=Download,-full%20dataset", 
                destfile= "Spatial_data/myc_map.zip", method= "libcurl", 
...
#Read in AM root association data - 10 arc-min resolution
AM_intensity <- rast("Spatial_data/myc_map/AM_intensity_colonization.tif") #%>%
  #aggregate(, fact = 3.33) #%>% #aggregate

#Make df and remove 0 values
AM_int_df <- as.data.frame(AM_intensity, xy=TRUE) %>%
  filter(AM_intensity_colonization > 0) #remove 0 values


#Overlay ecoregions onto raster
AM_int_sf <- st_as_sf(AM_int_df, coords = c("x", "y"), crs = st_crs(AM_intensity)) #convert to sf object
AM_int_sf <- st_transform(AM_int_sf, crs = 3395) #convert coord system to EPSG:3395 for World Mercator
AM_int_sf <- st_make_valid(AM_int_sf) #fix invalid geometries
AM_int_ecoregions <- st_join(ecoregions_sf, AM_int_sf, join = st_intersects) #join ecoregions to AM data

#Tried to put AM and crops together, but doesn't work because both have points instead of polygons
#AM_crops <- st_join(AM_sf, cropland_sf, join = st_intersects) #join cropland data to AM data

#Make into dataframe and summarise by ecoregion
AM_int_ecoregions_df <- as.data.frame(AM_int_ecoregions) %>% #make into dataframe
  dplyr::select(ECO_NAME, BIOME_NAME, geometry, AM_intensity_colonization) %>% #select relevant columns
  group_by(ECO_NAME, BIOME_NAME) %>% #group by ecoregion and biome
  summarise(AM_intensity = mean(AM_intensity_colonization, na.rm = TRUE)) #mean for ecoregion
...
```{r combine and clean dataframe}

#Calculate ecoregion area in square meters and convert to square kilometers
ecoregions_sf$area_sq_m <- st_area(ecoregions_sf)
ecoregions_sf$area_sq_km <- as.numeric(ecoregions_sf$area_sq_m) / 1e6

#Convert into data frame
ecoregions_df <- as.data.frame(ecoregions_sf, xy = TRUE) %>%
  dplyr::select(ECO_NAME, BIOME_NAME, area_sq_km) #select relevant columns

#Join ecoregions, ABG, and AM data, calculate Pg C per sq km
bio_regions <- merge(ecoregions_df, ABG_regions_df, by = c("ECO_NAME", "BIOME_NAME")) %>%
  merge(NPP_regions_df, by = c("ECO_NAME", "BIOME_NAME")) %>%
  merge(AM_roots_ecoregions_df, by = c("ECO_NAME", "BIOME_NAME")) %>%
  merge(AM_int_ecoregions_df, by = c("ECO_NAME", "BIOME_NAME")) %>%
  mutate(ABG_Pg_sqkm = ABG_Pg_mean * area_sq_km, #calculate ABG C in Pg per sq km
         AM_roots_Pg_sqkm = AM_roots_Pg_mean * area_sq_km) %>% #calculate AM C in Pg per sq km
...
bio_regions <- bio_regions %>%
  ...
  mutate(biome = #define biomes more closely
           case_when(BIOME_NAME == 
                     grepl("Temperate", BIOME_NAME) & #use grepl to find a substring
													 ...
													 grepl("puna", ECO_NAME, ignore.case = TRUE)
															 ~ "Montane Grasslands", 
													 grepl("Montane", BIOME_NAME) & 
													 grepl("páramo", ECO_NAME, ignore.case = TRUE)
															 ~ "Montane Grasslands", 
													 grepl("Montane", BIOME_NAME) & 
...
write_csv(bio_regions, "bio_regions.csv") #write to csv

#Summarise by biome
biome_summary <- bio_regions %>%
  group_by(biome) %>% #group by biome
  dplyr::select(biome, area_sq_km, NPP_Pg, ABG_Pg_sum, ABG_Pg_sqkm, 
                AM_roots_Pg_sum, AM_roots_Pg_sqkm, AM_intensity) %>% #select only relevant columns
  summarise_at(c("area_sq_km", "NPP_Pg", "ABG_Pg_sum", "ABG_Pg_sqkm", 
                 "AM_roots_Pg_sum", "AM_roots_Pg_sqkm"), sum, na.rm = TRUE) #%>%

write_csv(biome_summary, "biome_summary.csv") #write to csv

#Cropland area
crop_sqkm <- read.csv("crop_sqkm.csv") #update use calculated irrigated cropland from above

```

```{r clean spatial data, eval=FALSE}

#Adjust extents to match and combine ABG with AM belowground data
#all.equal(extent(AM_ecoregions), extent(ABG_regions))
#biomass_regions_sf <- st_join(AM_ecoregions, ABG_regions, join=st_intersects) #%>% #join ecoregions to ABG and BG data

#biomass_regions_sf <- st_union(biomass_regions_sf, ecoregions_sf) #join ecoregions and cropland data
  
#Calculate area in square meters and convert to square kilometers
ecoregions$area_sq_m <- st_area(ecoregions)
ecoregions$area_sq_km <- as.numeric(ecoregions$area_sq_m) / 1e6

#Extract coordinates from geometry column
biomass_regions_df <- as.data.frame(biomass_regions_sf) %>%
  dplyr::select(ECO_NAME, BIOME_NAME, area_sq_km, geometry, aboveground_biomass_carbon_2010, 
                belowground_biomass_carbon_2010, cropland_2019) %>% #select relevant columns
  rename("ABG_C" = "aboveground_biomass_carbon_2010",
         "BG_C" = "belowground_biomass_carbon_2010") %>%
  mutate(long = st_coordinates(geometry)[, 1], #extract longitude
         lat = st_coordinates(geometry)[, 2]) #extract latitude

#Create new column with more specific ecoregion names
biomass_regions_df <- biomass_regions_df %>%
  #na.omit() %>%
  filter(ABG_C > 0) %>% #remove 0 values
  filter(BG_C > 0) %>% #remove 0 values
  #filter(!grepl("Desert", ECO_NAME, ignore.case = TRUE)) %>% #filter out deserts
  #filter(!grepl("Rock and Ice", ECO_NAME, ignore.case = TRUE)) %>% #filter out rock and ice
  mutate(across('BIOME_NAME', str_replace, 'Mediterranean Forests, Woodlands & Scrub', 'Mediterranean Forests')) %>%
  mutate(biome = #define biomes more closely
           case_when(BIOME_NAME == NA & cropland2019 > 0 ~ "Cropland",
                     grepl("Temperate", BIOME_NAME) & #use grepl to find a substring
													 grepl("Grass", ECO_NAME, ignore.case = TRUE)
															 ~ "Temperate Grasslands", #set value in new column 
													 grepl("Temperate", BIOME_NAME) & 
...
#Cropland area
#cropland <- data.frame(biome = "Cropland", area_sq_km = 12440000) #1,244 million hectares
crop_sqkm <- read.csv("crop_sqkm.csv") #update use calculated irrigated cropland from above

#Create columns with area adjusted to AM proportions and EcM proportions
Steidinger <- read.csv('AM_EM_proportions.csv') #read in data for myc proportions

biomass_regions_df <- biomass_regions_df %>%
  add_row(biome = "Cropland", area_sq_km = 12440000, ABG_C = 0, BG_C = 0) %>% #add cropland area
  bind_rows(cropland) %>% #bind rows of landcover to ecoregions
...
ecoregions_sum <- biomass_regions_df %>%
  group_by(biome) %>% #group by biome
...
To estimate the contribution of AM fungal biomass to global C pools, I used the estimated amount of AM extra-radical hyphal biomass C in grams per square meter (g/m^2) from [Soudzilovskaia et al. 2015](https://onlinelibrary.wiley.com/doi/10.1111/nph.13447), scaled it to total biome area, and converted the units to Pg of C. 

Additionally, I included estimated C allocation from plants to mycorrhizae per year based on vegetation type averages from [Hawkins et al. 2023](https://doi.org/10.1016/j.cub.2023.02.027) and scaled to total biome area for a Pg of C per year estimate.
...
Regions were defined based on latitude, from the [Encyclopedia of World Climatology, 2005](https://link.springer.com/referencework/10.1007/1-4020-3266-8). I used a reference table from [Chapter]() linking soil orders to ecosystem types to further classify biomes in the SOM dataset. A mean annual precipitation of 2000mm or more was considered "moist" for tropical and subtropical broadleaf forests, while less than 2000mm was considered "dry" ([Siyum 2020](https://doi.org/10.1186/s13717-020-00229-6)). Mediterranean forests were defined as having a mean annual precipitation of 900mm or less, within the 30 and 45 degree latitute range ([Encyclopaedia Britannica 2025](https://www.britannica.com/science/Mediterranean-climate)). Xeric shrublands were defined as having a mean annual precipitation of between 200mm and 1000mm ([Neffar and Chenchouni, 2025](https://doi.org/10.1016/B978-0-443-36569-0.00016-1)). The data was filtered to remove fallow land, and only the top 30cm of soil was used for analysis. The data was then merged with the global cover data to estimate C pools in Pg of C.
...
SOMC <- read_csv("MOC_synthesis.csv") %>%
  ...
  mutate(region = #add region based on latitude
           case_when(Lat < 23.5 & Lat > -23.5 ~ "Tropical",
                     ...
                     MAP <= 900 & Lat < 45 & Lat > 30 | 
                       MAP <= 900 & Lat > - 45 & Lat < -30 ~ "Mediterranean",
  ...
  mutate(biome = #define biomes more closely
           case_when(Vegetation == "Grassland" & region == "Temperate" 
                     ...
                     ~ "Tropical & Subtropical Shrublands",
										 Vegetation == "Shrubland" & MAP > 200 & MAP < 1000
                     ~ "Xeric Shrublands",
										 grepl("Forest", Vegetation, ignore.case = TRUE) & region == "Mediterranean" 
...
SOMC_top30_means <- SOMC %>%
  ...
  select(biome, Bottom.depth, Bulk.C, MAOM_C, POM_C) %>%
  group_by(biome) %>%
...
### Table of biome area and C pools
```{r table}
...
ggplot(AM_cover, aes(x=reorder(biome, -area_sq_km), y=AMbio_C, fill=biome)) + 
    geom_bar(position="dodge", stat = "identity", width = 0.7, colour="black") + #add proportion bars with black outline
    scale_x_discrete(guide = guide_axis(angle = 45)) +
    labs(title="AM Fungal Biomass Carbon by Biome",
       x="Biome",
       y="AM Biomass C (Pg C)") +
    scale_fill_d3("category20c") +
    theme_minimal() 

ggsave("AM_bioC_052025.png", path = "Figures", #save to folder within working directory
       width = 8, height = 5, dpi = 300, bg=NULL) #save figure
