...
Steidinger <- read.csv('AM_EM_proportions.csv') #read in data for AM proportions

#Create new column with more specific ecoregion names
#case when function from dplyr to define values in a new column based on conditions - with substrings
ecoregions_area <- ecoregions_area %>%
  dplyr::select(ECO_NAME, BIOME_NAME, area_sq_km) %>% #select only relevant columns
  filter(BIOME_NAME != "Boreal Forests/Taiga") %>% #filter out Boreal Forests
  ...
  mutate(biome = #define biomes more closely
           case_when(grepl("Temperate", BIOME_NAME) & #use grepl to find a substring
													 ...
													 grepl("Tropical", BIOME_NAME) & #use grepl to find a substring
													 grepl("Grassland", ECO_NAME, ignore.case = TRUE)
															 ~ "Tropical Grasslands", #set value in new column 
													 grepl("Tropical", BIOME_NAME) & 
													 grepl("Savanna", ECO_NAME, ignore.case = TRUE)
															 ~ "Tropical Savannas",  
													 grepl("Tropical", BIOME_NAME) & 
													 grepl("miombo", ECO_NAME, ignore.case = TRUE)
															 ~ "Tropical Savannas",  
													 grepl("Tropical", BIOME_NAME) & 
													 grepl("mopane", ECO_NAME, ignore.case = TRUE)
															 ~ "Tropical Savannas", 
													 grepl("Tropical", BIOME_NAME) & 
													 grepl("Shrub", ECO_NAME, ignore.case = TRUE)
															 ~ "Tropical Shrublands", 
													 ...
													 grepl("Deserts & Xeric Shrublands", BIOME_NAME)
													     ~ "Xeric Shrublands",
		                           .default = BIOME_NAME)) %>% #set default value of new column
  mutate(AM_area = #multiply by proportion of AM
           case_when(grepl("Tundra", biome, ignore.case = TRUE),
                     ~ (area_sq_km*((Steidinger$median[Steidinger$group_id == "Tundra_AM"])/100)),
            .default = area_sq_km))
